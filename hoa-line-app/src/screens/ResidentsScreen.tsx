...
const AddResidentModal: React.FC<AddResidentModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleInviteFriend = async () => {
    try {
      ...
      if (!liff.isLoggedIn()) {
        ...
        if (LIFF_ID) {
          try {
            ...
            console.log('LIFF 已初始化或初始化失敗:', initError);
          }
        }
      }
      ...
      console.log('是否在 LINE 內:', isInClient);
      console.log('shareTargetPicker 可用:', liff.isApiAvailable('shareTargetPicker'));
      
      // 生成邀請連結（使用 LIFF URL 格式，讓用戶在 LINE App 內打開）
      ...
      if (!liff.isApiAvailable('shareTargetPicker')) {
        // 如果不在 LINE 內，提供替代方案
        const lineShareUrl = `https://line.me/R/share?text=${encodeURIComponent('邀請您加入幸福社區住戶登記：' + inviteUrl)}`;
      ...
      // 備用方案：使用 LINE 分享連結（仍使用 LIFF URL）
      const LIFF_ID = (import.meta.env.VITE_LIFF_ID || '').trim();
      const inviteUrl = `https://liff.line.me/${LIFF_ID}?register=true&unit=${encodeURIComponent(formData.unit || '待填寫')}`;
      const lineShareUrl = `https://line.me/R/share?text=${encodeURIComponent('邀請您加入幸福社區住戶登記：' + inviteUrl)}`;
      window.open(lineShareUrl, '_blank');
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="新增住戶">
      <div className="p-5 space-y-4">
        ...
        {showInviteSuccess && (
          <div className="bg-green-50 text-green-700 px-4 py-3 rounded-xl text-center text-sm">
            ✓ 邀請已發送！<br />
            <span className="text-xs">等待住戶點擊連結後，會自動出現在清單中</span>
          </div>
        )}

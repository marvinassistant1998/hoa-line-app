...
import { Header, Card, ListItem, Badge, Modal } from '../components.jsx';
...
export const SettingsScreen = () => {
  ...
  const usage = { meetings: 2, maxMeetings: 3, reports: 1, maxReports: 2 };
  const admins = [
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] pb-24">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xl font-bold text-[#1D1D1F]">
                {currentPlan === 'free' ? '免費版' : currentPlan === 'pro' ? '進階版' : '專業版'}
              </p>
            </div>
            <button
      ...
      {/* 管理者管理 Modal */}
      <AdminModal isOpen={showAdminModal} onClose={() => setShowAdminModal(false)} admins={admins} />
    </div>
  );
};
...
const SettingRow = ({ label, value, isLast = false }) => (
...
const SettingLink = ({ label, isLast = false }) => (
  <button className={`w-full flex items-center justify-between py-3 ${!isLast ? 'border-b border-[#E8E8ED]' : ''}`}>
...
const PlanModal = ({ isOpen, onClose, currentPlan }) => {
  ...
  const plans = [
    ...
    },
    {
      ...
      name: '專業版',
      price: 'NT$ 2,490/月',
      features: ['無限 AI 會議紀錄', '無限 AI 財務報表', '10 位管理者', '專屬客服', 'API 整合'],
    },
  ];
            ...
            <ul className="space-y-1">
              {plan.features.map((f, i) => (
                <li key={i} className="text-sm text-[#86868B] flex items-center gap-2">
                  <Icons.check className="w-4 h-4" color="#34C759" />
                  {f}
                </li>
              ))}
            </ul>
...
const AdminModal = ({ isOpen, onClose, admins }) => {
  if (!isOpen) return null;
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="管理者設定">
      <div className="p-5 space-y-4">
        ...
        <div className="space-y-2">
          {admins.map(admin => (
            <div key={admin.id} className="flex items-center justify-between p-3 bg-[#F5F5F7] rounded-xl">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#06C755]/10 flex items-center justify-center">
                  <span className="font-medium text-[#06C755]">{admin.name[0]}</span>
                </div>
              ...
              <div className="flex items-center gap-2">
                {admin.isOwner ? (
                  ...
                  <button className="p-2 text-[#FF3B30]">
                    <Icons.trash className="w-5 h-5" color="#FF3B30" />
                  </button>
                )}
              </div>
            </div>
